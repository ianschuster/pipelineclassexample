// DotNetLoader.cpp : Defines the exported functions for the DLL.
//
...
DOTNETLOADER_API int fnDotNetLoader(void)
{
    return 0;
}
...
int RunDotNetProgram1()
#else
int main(int argc, char* argv[])
#endif
{
    wchar_t argv[1024];
    wcsncpy_s(argv, L"C:\\Users\\polek\\source\\repos\\JNI_JVM_example\\DotNetLIb\\bin\\Debug\\net5.0\\DotNetLIb.dll", 1024);

    // Get the current executable's directory
    // This sample assumes the managed assembly to load and its runtime configuration file are next to the host
    char_t host_path[MAX_PATH];
#if WINDOWS
    auto size = ::GetFullPathNameW(argv, sizeof(host_path) / sizeof(char_t), host_path, nullptr);
    assert(size != 0);
#else
